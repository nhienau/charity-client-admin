...
export async function getLecturer(id) {
  try {
    const res = await fetch(
      `${import.meta.env.VITE_API_URL}/lecturer/get?id=${id}`,
    );
    const data = await res.json();
    return data;
  } catch (e) {
...
export async function updateLecturer(lecturer) {
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/lecturer/update`, {
      ...
      body: JSON.stringify(lecturer), // thông tin giảng viên mới
    });
...
export async function deleteLecturer(id) {
  try {
    const res = await fetch(
      `${import.meta.env.VITE_API_URL}/lecturer/delete?id=${id}`,
      {
        method: "DELETE",
      },
    );
